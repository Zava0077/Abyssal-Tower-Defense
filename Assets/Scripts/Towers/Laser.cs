...
public class Laser : BulletEffects
{
    ...
    public override void Travel(GameObject proj)
    {
        ...
        proj.GetComponent<Projectile>().transform.position += proj.transform.right * Vector3.Distance(proj.GetComponent<Projectile>().position, proj.GetComponent<Projectile>().target) / 8;
