...
public class Homing : BulletEffects
{
    ...
    public override void Travel(GameObject proj)
    {
        ...
        {
            proj.transform.rotation = Quaternion.Lerp(proj.transform.rotation, Quaternion.LookRotation(Vector3.RotateTowards(proj.transform.forward, enemy.transform.position - proj.transform.position, 3.14f, 0f)), (0.05f * 35f) / proj.GetComponent<Projectile>().projSpeed);
